...
export const rbac = (allowedRoles = [], requiredPermissions = []) => {
  return async (req, res, next) => {
    try {
      ...
      if (!req.user) {
        console.log('❌ RBAC Error: No user found in request - authentication required');
      ...
      console.log(`👤 User Info - ID: ${userId}, Role: ${userRole}, Lab: ${labId}`);
      ...
      if (!hasRequiredRole) {
        ...
        console.log(`   User role: ${userRole} (level ${ROLE_HIERARCHY[userRole]})`);
        console.log(`   Required roles: ${allowedRoles.join(', ')}`);
      ...
      if (requiredPermissions.length > 0) {
        ...
        if (!hasAllPermissions) {
          ...
          console.log(`❌ RBAC Error: Missing permissions: ${missingPermissions.join(', ')}`);
      ...
      if (userRole !== 'super_admin' && req.params.labId) {
        ...
        if (labId && labId.toString() !== requestedLabId) {
          console.log(`❌ RBAC Error: Lab access denied`);
          console.log(`   User's lab: ${labId}`);
          console.log(`   Requested lab: ${requestedLabId}`);
      ...
      console.log(`   Permissions: ${ROLE_PERMISSIONS[userRole]?.join(', ') || 'None'}`);
