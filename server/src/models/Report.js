...
const TemplateFieldSchema = new mongoose.Schema({
  ...
  fieldType: {
    type: String,
    enum: ['text', 'number', 'select', 'multiselect', 'textarea', 'date', 'image'],
    required: true
  },
...
const ReportSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['draft', 'in_review', 'pending_approval', 'approved', 'delivered', 'amended', 'cancelled'],
    default: 'draft',
    index: true
  },
  ...
  if (this.isNew) {
    try {
      ...
      if (!this.reportId) {
        ...
        this.reportId = `RPT${labCode}${dateStr}${String(count + 1).padStart(4, '0')}`;
      }
      
      // Generate report number if not provided
      if (!this.reportNumber) {
        this.reportNumber = `${this.reportId}-${crypto.randomBytes(2).toString('hex').toUpperCase()}`;
      }
...
// Methods
ReportSchema.methods.updateStatus = function(newStatus, updatedBy, comments = '') {
...
ReportSchema.methods.addApproval = function(approvedBy, level, signature = null, comments = '') {
  this.approvals.push({
    approvedBy,
    level,
    signature,
...
ReportSchema.methods.amendReport = function(amendedBy, reason, newResults = null) {
  this.isAmended = true;
...
ReportSchema.methods.logAccess = function(userId, action, ipAddress = '', userAgent = '') {
  this.accessLog.push({
    accessedBy: userId,
    action,
    ipAddress,
...
ReportSchema.methods.calculateTurnaroundTime = function() {
  if (this.createdAt && this.reportedAt) {
    const turnaroundHours = Math.round((this.reportedAt - this.createdAt) / (1000 * 60 * 60));
...
ReportSchema.methods.isOverdue = function() {
  if (!this.createdAt || ['approved', 'delivered', 'cancelled'].includes(this.status)) {
    return false;
  }
  
  const hoursElapsed = (new Date() - this.createdAt) / (1000 * 60 * 60);
  const maxHours = this.priority === 'stat' ? 2 : this.priority === 'urgent' ? 6 : 24;
...
ReportSchema.methods.canAccess = function(userId, action = 'view') {
  ...
  const restriction = this.accessRestrictions.find(r => r.userId.toString() === userId.toString());
  if (restriction) {
    return restriction.permission === action || 
           (action === 'view' && ['download', 'print', 'share'].includes(restriction.permission));
  }
